...
% Let floats fill text pages instead of piling onto a float-only page after the references.
...
\begin{tikzpicture}[
  ...
  fno/.style={box, fill=blue!8}, cnn/.style={box, fill=orange!12},
  ev/.style={box, fill=green!8, align=left}, >={Stealth[]}, thick,
]
\node (u0) [box] at (0,0) {Input\\ $u_0$};
\node (lift) [fno] at (2.4,1.05) {Lift $P$};
\node (four) [fno] at (5.0,1.05) {\shortstack{Fourier layer $\times L$\\[1pt] \scriptsize FFT $\cdot$ keep low-$k$ modes $\hat W_k$ $\cdot$ iFFT $+$ local $1{\times}1$}};
\node (proj) [fno] at (8.0,1.05) {Project $Q$};
\node (conv) [cnn] at (5.0,-1.05) {\shortstack{Local conv $\times L$\\[1pt] \scriptsize kernel 3, circular padding, residual}};
\node (pred) [box] at (10.7,0) {\shortstack{Prediction\\ $u(T)$}};
\node (ev) [ev] at (14.2,0) {\shortstack[l]{\textbf{Evaluation}\\[1pt] \scriptsize band error $e^{\mathrm{abs}}_b$, $\phi_b$\\ \scriptsize resolution $128{\to}256$\\ \scriptsize regime sweep $\nu$\\ \scriptsize Track B: FFT correlation}};
\node[font=\scriptsize\itshape, blue!55!black] at (5.0,2.05) {FNO branch (spectral, global)};
\node[font=\scriptsize\itshape, orange!60!black] at (5.0,-2.0) {CNN branch (local)};
\draw[->] (u0) |- (lift);
\draw[->] (lift) -- (four);
\draw[->] (four) -- (proj);
\draw[->] (u0) |- (conv);
...
\draw[->] (pred) -- (ev);
\end{tikzpicture}
\caption{Benchmark architecture. An input field is mapped to the PDE solution by two matched budget model families, a Fourier neural operator whose spectral layer performs a global convolution as a multiplication of truncated modes, and a purely local convolutional network. Both are scored by the same evaluation battery, whose core is the band resolved error read together with the per band energy fraction, plus zero shot resolution generalization, the viscosity regime sweep, and the Track B correlation localizer.}
\label{fig:arch}
\end{figure*}
...
All comparisons are at a matched parameter budget of about $45{,}000$ to $46{,}000$ trainable parameters and a shared training protocol: identical optimizer (Adam), a cosine schedule, $400$ steps, batch size $16$, and identical data splits and normalization. Each cell is averaged over three seeds, reported as mean and standard deviation. Track A uses a spatial resolution of $128$ for training and evaluates zero shot at $256$. We report two evaluation regimes throughout. \emph{In distribution} means held out test samples drawn from the same distribution as training, that is at the training resolution and within the training viscosity range, which the model never saw as individual examples but which match the training conditions. \emph{Out of distribution} means the same physics at doubled grid resolution, a discretization the model never encountered. The difference between the two therefore measures resolution generalization, not merely test accuracy. These are deliberately small, single machine CPU runs intended for reproducibility and methodological demonstration, not a large scale ranking; see limitations.

\subsection{Implementation and reproducibility}
The Burgers reference solver, all models, the metrics and the training loop are implemented in PyTorch and run on a single CPU. The full grid reported here, four viscosities times two architectures times three seeds for Track A together with the Track B signal to noise sweep, completes in under four minutes on a laptop class processor. Model widths are set by a parameter matched width search so that the Fourier operator and the convolutional network agree to within a few percent in trainable parameter count, which is what lets us attribute differences to architecture rather than capacity. The code, the full specification and the exact configuration that produced every number in this paper are released alongside it, and a machine readable results file records the raw per seed measurements as provenance.

\section{Results}
Table \ref{tab:tracka} reports Track A numerically and Fig.~\ref{fig:results} the same evidence visually. Three findings stand out and are, we stress, honest rather than convenient.

\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/results}
\caption{Benchmark evidence. (a) In distribution versus zero shot $2\times$ resolution relative $L^2$ error across viscosity: the FNO curves nearly coincide, whereas the CNN error jumps to $0.21$--$0.24$ at doubled resolution. (b) Resolution degradation ratio ($2\times$/in), log scale: the FNO stays near unity, the CNN degrades by $4.5$ to $13$. (c) Track B, template localization error versus signal to noise ratio: FFT cross correlation is sub pixel at high SNR and crosses the trivial center guess baseline near unity. Error bars in (a) are the standard deviation over three seeds.}
\label{fig:results}
\end{figure*}
...
A second thread connects Track B to the representation question. The classical result that the perceptually salient structure of a signal resides in its Fourier phase rather than its magnitude \cite{oppenheim1981} suggests that the natural axis separating a coherent pattern from broadband noise is phase coherence rather than a single frequency band, since white noise is spectrally flat while natural signal energy is concentrated, so their ratio, not their location, is what a filter can exploit. Our correlation localizer rests on exactly this, and its sharp collapse near unit signal to noise ratio marks where that separation ceases to be informative. A controlled study of phase based separability, and of adversarial as opposed to merely stochastic perturbations, is a natural next step.
